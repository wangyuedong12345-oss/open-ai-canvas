...
func runDeclarativeAgentTask(ctx context.Context, input canvasGenerationInput, adapter protocol.AgentAdapter) (map[string]interface{}, error) {
	...
	wantsStream := input.StreamText
	if body := protocolBodyObject(spec.Body); body != nil {
		stream, _ := body["stream"].(bool)
		wantsStream = wantsStream || stream
	}
	if knownWire {
	...
	if wantsStream {
		...
		data, mimeType, err := executeProtocolBinaryRequestWithConsumerUntil(ctx, input.Config, spec, parser.consume, parser.doneReading)
		if err != nil {
			return nil, err
		}
		...
		parsed, err := adapter.ParseAgent(ctx, data)
		if err != nil {
			return nil, err
		}

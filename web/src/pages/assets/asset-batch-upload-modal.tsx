...
export function AssetBatchUploadModal({ open, defaultFolderId, folders, onClose, onComplete }: { open: boolean; defaultFolderId: string; folders: AssetFolder[]; onClose: () => void; onComplete: () => Promise<void> }) {
    ...
    return <AppModal className="library-modal library-batch-upload-modal" title="批量上传图片" open={open} onCancel={close} footer={null} flush>
        <div className="batch-upload-modal-content">
            <div className="batch-upload-fields">
                <label className="batch-upload-field">
                    <span className="batch-upload-field-label">业务分类</span>
                    <Select className="w-full" value={category} options={ASSET_CATEGORY_OPTIONS} onChange={(value) => setCategory(value)} />
                </label>
                <label className="batch-upload-field">
                    <span className="batch-upload-field-label">自定义分类</span>
                    <Select className="w-full" value={folderId} options={folderOptions} onChange={setFolderId} />
                </label>
                <label className="batch-upload-field">
                    <span className="batch-upload-field-label">公共标签</span>
                    <Select mode="tags" className="w-full" value={tags} tokenSeparators={[",", "，"]} onChange={setTags} placeholder="输入后回车" />
                </label>
            </div>
            ...
            <div className="batch-upload-modal-actions"><Button onClick={close} disabled={uploading}>取消</Button><Button type="primary" icon={<UploadCloud className="size-4" />} onClick={() => void uploadBatch()} disabled={!items.some((item) => item.status === "queued" || item.status === "error")} loading={uploading}>{failedItems.length ? `重试失败项 (${failedItems.length})` : "开始上传"}</Button></div>
        </div>
    </AppModal>;
}
